...
Simple and debuggable. One agent (`logan`), one skill (`local-openclaw-cardano`), one mission. No multi-agent coordination overhead.

## 2. Bash+curl for API Calls

No MCP server needed. Matches OpenClaw conventions. Every local OpenClaw API call is a simple `curl` with `Authorization: Bearer $OPENAI_API_KEY`. Easy to debug, log, and rate-limit.
...
- **24 cycles per day**, no downtime — local OpenClaw is a global AI agent platform, there are no off-hours
...
- This volume makes Logan the most active crypto educator on local OpenClaw by a wide margin
